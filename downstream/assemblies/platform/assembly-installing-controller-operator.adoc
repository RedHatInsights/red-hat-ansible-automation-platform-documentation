...
* For Controller, a default 'StorageClass' must be configured on the cluster for the operator to dynamically create needed PVC's. This is not necessary if an external PostgreSQL database is configured.
