...
Custom repositories:: any user with write permissions on the repository can publish content to these repositories. These repositories are not displayed on the approval dashboard. They can show up on the search page depending on whether search is enabled by the owner of the repository. Custom repositories can be private repositories (ie only users with view permissions can see them) or public (all users can see them). See [xref to Cross-repo search].

Staging repositories are marked with the 'pipeline=staging' label. By default automation hub ships with one staging repository that is automatically used when a repository is not specified for uploading collections. However, users can create new staging repositories from the repository creation screen. See [xref to Creating a repository].
