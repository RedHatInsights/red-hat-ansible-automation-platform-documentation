...
+
The `ids_rules_file` variable specifies a defined location for the `local.rules` file, while the `ids_rule_state` variable indicates if the rule should be created if it does not already exist.
...
To verify that your IDS rules were successfully created, SSH to the Snort server and look for the content of the ``/etc/snort/rules/local.rules` file.
