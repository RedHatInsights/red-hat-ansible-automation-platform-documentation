...
. Copy the URL links for {ControllerName} and {HubName} and save them. The names for the links are _automationControllerUrl_ and _automationHubUrl_.
. In a browser, navigate to the {ControllerName} URL and log in using the following credentials:
  * *Username*: `admin` 
