...
To set up a high availability (HA) deployment of {HubName} on SELinux, create two mount points for `/var/lib/pulp` and `/var/lib/pulp/pulpcore_static`, then assign the appropriate SELinux contexts to each. You must add the context for `/var/lib/pulp/pulpcore_static` and run the Ansible Automation Platform installer before adding the context for `/var/lib/pulp`.
