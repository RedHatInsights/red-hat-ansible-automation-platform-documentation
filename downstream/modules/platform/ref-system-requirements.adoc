...
[NOTE]
====
You must install Ansible using a package manager such as `yum`, and the latest stable version must be installed for {PlatformName} to work properly. Ansible version 2.9 is required for versions 3.8 and later.
====
...
//There is text to explain this entry. 
