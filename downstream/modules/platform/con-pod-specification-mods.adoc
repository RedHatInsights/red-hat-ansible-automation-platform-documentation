...
The Kubernetes concept of a pod is one or more containers deployed together on one host, and the smallest compute unit that can be defined, deployed, and managed.
...
An example definition of a pod follows, that provides a long-running service. 
It demonstrates many features of pods, most of which are discussed in other topics and thus only briefly mentioned here:
...
The pod used to run jobs in a kubernetes-based cluster using {ControlerNameShort} can be modified by editing the pod specification in the {ControllerNameShort} UI.  
