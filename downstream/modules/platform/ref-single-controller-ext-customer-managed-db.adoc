...
[role="_abstract"]
This example describes how you can populate the inventory file to install {PlatformName}. This installation inventory file includes a single {ControllerName} node with an external database on a separate node that is not managed by the platform installer.
