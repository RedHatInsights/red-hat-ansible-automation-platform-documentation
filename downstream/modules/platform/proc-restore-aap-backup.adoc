...
Running the `./setup.sh` script with the `restore_backup_file` flag will migrate content from the backup file of your original instance to the new instance, effectively migrating all job histories, templates, and other {PlatformNameShort} related content.

. Run the following command:
+
----
./setup.sh -r -e ‘restore_backup_file=/mybackup/tower-backup-latest.tar.gz’ -e ‘user_compression=True’ -e @credentials.yml -r -- --ask-vault-pass
----
