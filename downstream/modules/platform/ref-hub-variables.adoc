...
Default = `false`.
| *`automationhub_sso_console_admin_password`* | SSO admin password.
| *`automationhub_sso_console_keystore_file`* | Keystore file to install in SSO node.

`/path/to/sso.jks`
| *`automationhub_sso_host`* | {CatalogNameStart} requires SSO and SSO administration credentials for
authentication. 
...
| *`automationhub_sso_keystore_password`* | A keystore password is required for https enabled SSO.

The default install deploys SSO with `sso_use_https=True`
| *`automationhub_sso_redirect_host`* | If `sso_redirect_host` is set, it is used by the application to connect to SSO for authentication. 

This must be reachable from client machines.
| *`automationhub_sso_use_https`* | If Single Sign On is used
