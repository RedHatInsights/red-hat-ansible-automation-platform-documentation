...
If you do not want to install one type of content, there are two options:

* Set `automationhub_seed_collections` to `false`, or

* Set `automationhub_seed_collections` to `true` and `automationhub_collection_seed_repository` to the type of content you do want to include.
...
* Set `automationhub_seed_collections` to `false` to disable the seeding, or
