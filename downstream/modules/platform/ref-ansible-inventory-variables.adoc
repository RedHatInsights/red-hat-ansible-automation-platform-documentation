...
| *`ansible_shell_executable`* | This sets the shell the ansible controller will use on the target machine, and overrides the executable in `ansible.cfg` which defaults to `/bin/sh`. 

You should only change it if is not possible to use `/bin/sh`, that is, if `/bin/sh` is not installed on the target machine or cannot be run from sudo.
