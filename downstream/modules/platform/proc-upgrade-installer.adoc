...
To upgrade your instance of Ansible Tower to {PlatformNameShort} {PlatformVers}, start by copying the `inventory` file from your original Tower instance to your new instance, then running the installer. The {PlatformName} installer will detect a pre-{PlatformVers} and offer an upgraded `inventory` file for you to continue with the upgrade process:
...
<3> Specifies hop nodes, which route traffic to other execution nodes. Hop nodes cannot execute automation.
