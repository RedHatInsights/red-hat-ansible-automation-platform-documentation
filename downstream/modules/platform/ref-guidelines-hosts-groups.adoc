...
* The database server should be on the same network or in the same datacenter as the {ControllerNameStart} server for performance reasons.
...
.{ControllerNameStart}
* {ControllerNameStart} will not configure replication or failover for the database that it uses, although automation controller should work with any replication that you have. 
