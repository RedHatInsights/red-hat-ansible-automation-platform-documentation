...
NOTE: The setup bundle includes additional components such as the AAP RPMs, and the default execution environment (EE) images which makes installing AAP easier in a disconnected environment. The setup bundle tarball is preferred over the standard setup tarball for disconnected networks.
