...
When migrating from {PlatformName} 2.0 or earlier to {PlatformName} 2.1 or later, you can use your existing automation hub API token or generate a new token. In the inventory file, edit the following fields before running the {PlatformName} installer setup script `setup.sh`:

If you are upgrading from {PlatformName} 2.0 or earlier, you must perform one of the following tasks:
