...
* Provide a fully qualified domain name (FDQN) for the `[automationhub]` host to ensure users can sync and install content from {HubName} from a different node. Do not use 'localhost'.
...
[automationhub]
<__automation_hub_FQDN__> ansible_python_interpreter=/usr/bin/python3
