...
+
----
$ sudo dnf install --enablerepo=ansible-automation-platform-{PlatformVers}-for-rhel-8-x86_64-rpms ansible-navigator
----
+
v.{PlatformVers} for RHEL 9 for x86-64
+
----
$ sudo dnf install --enablerepo=ansible-automation-platform-{PlatformVers}-for-rhel-9-x86_64-rpms ansible-navigator
----
